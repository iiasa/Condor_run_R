...
This can happen when you have already submitted ongoing runs with very many jobs, more than configured by the `MAX_JOBS_RUNNING` configuration parameter which on machines with much memory defaults to 2000. If you really need to submit that many jobs, set `MAX_JOBS_RUNNING` to a higher value in the `condor_config.local` file of the HTCdonro installation on your submit machine. You can check this configuration value by issuing `condor_config_val MAX_JOBS_RUNNING`.

Another likely possibility is that the cluster is not collecting the seed jobs on account of a confused networking state. This can happen when your machine has hibernated for example. Resolve this by restarting the Condor background processes on the submit machine. You can do so by issuing `condor_restart -master` from a command prompt. If thereafter submission still fails in the same way and you have administrator/root rights on the submit machine, you can instead try restarting the Condor service via the Services application on Windows or via [`systemctl restart condor.service`](https://manpages.debian.org/bullseye/systemctl/systemctl.1.en.html) on Linux. If you have no administrator/root rights, try rebooting the submit machine.
...
Some error occurred and the affected jobs got released for retrying. This can happen when [`JOB_RELEASES`](configuring.md#job_releases) is set to a value larger than zero. This is useful when the error is on account of a transient condition such as an execute node being shut down or a network outage. Then jobs encountering the error will be rescheduled and likely succeed, removing the need for laboriously restarting just the failed jobs or the whole run.

However, when the error is intrinsic to the job itself, for example a coding error, retrying is not useful since the job will go on hold every time. It is therefore makes sense use `JOB_RELEASES = 0` (the default value) until you are sure that your jobs are working without problems.
