...
A likely possibility is that the cluster is not collecting the seed jobs on account of a confused networking state. This can happen when your machine has hibernated for example. Resolve this by restarting the Condor background processes on the submit machine. You can do so by by issuing `condor_restart -master` from a command prompt. If thereafter submission still fails in the same way and you have administrator/root rights on the submit machine, you can instead try restarting the Condor service via the Services application on Windows or via [`systemctl restart condor.service`](https://manpages.debian.org/bullseye/systemctl/systemctl.1.en.html) on Linux. If you have no administrator/root rights, try rebooting the submit machine.
