...
When you do not have the rights to log in to execute hosts to analyze held jobs in-situ, setting the [`RETAIN_BUNDLE`](configuring.md#retain_bundle) option to `TRUE` can assist. This will preserve a copy of the 7-Zip bundle sent to the execute hosts in the run-specific directory that also holds the log files. Unzip that bundle and try to run the job locally to see if you can reproduce the problem.
