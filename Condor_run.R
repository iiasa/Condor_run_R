...
SEED_JOB_TEMPLATE <- c(
  ...
  "periodic_remove = (NumJobMatches > 1) || ((JobStatus == 1) && (time() - EnteredCurrentStatus > 120 ))", # if seed job is matched multiple times or remains idle for more than 2 minutes, remove it as presumably the execution point is not responding
  "",
...
if (tools::file_ext(file_arg) == "7z") {
  ...
  if (GET_GDX_OUTPUT) {
    ...
    GDX_OUTPUT_DIR <- str_glue(GDX_OUTPUT_DIR)
    ...
    if (str_detect(GDX_OUTPUT_DIR, '[<>|:?*" \\t\\\\]')) stop(str_glue("Configured GDX_OUTPUT_DIR has forbidden character(s) after {} expansion! Use / as path separator."))
